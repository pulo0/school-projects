...
<body>
    ...
    <main>
        <img src="pliki/crested.jpg" alt="Świnka morska rasy crested">
        <?php
        $sql = 'SELECT DISTINCT data_ur, miot, rasa FROM swinki INNER JOIN rasy ON rasy.id = swinki.rasy_id WHERE rasy.id LIKE 7';
        $query_sql = $conn->query($sql);
        ...
        <?php
        $sql = 'SELECT imie, cena, opis FROM swinki INNER JOIN rasy ON rasy.id = swinki.rasy_id WHERE rasy.id LIKE 7';
        $query_sql = $conn->query($sql);
